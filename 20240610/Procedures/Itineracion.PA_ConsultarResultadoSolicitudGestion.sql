...
-- Descripción :			<Permite consultar la información de un resultado de solicitud de una itineración de Gestión>
-- =============================================================================================================================================================================
...
-- Modificación:			<24/06/2021><Ronny Ramírez R.> <Se aplica filtro por número de expediente en join con tabla SolicitudExpediente para que traiga un solo resultado>
-- =============================================================================================================================================================================
CREATE PROCEDURE [Itineracion].[PA_ConsultarResultadoSolicitudGestion]
...
BEGIN
	...
	SELECT			NEWID()																										AS Codigo,
					...
					'SplitSolicitudExpediente'																					AS SplitSolicitudExpediente,
					S.TU_CodSolicitudExpediente																					AS Codigo,
					'SplitOtros'																								AS SplitOtros,
					COALESCE(F.TC_UsuarioRed, @L_UsuarioCreaXDefecto)															AS UsuarioRed
		FROM		@L_XML.nodes('(/*/DCARTD9)')		AS X(Y)
		LEFT JOIN	@L_XML.nodes('(/*/DACO)')			AS A(B)
		ON			X.Y.value('(IDACOREF)[1]','INT')	= A.B.value('(IDACO)[1]', 'INT')
		INNER JOIN	Expediente.SolicitudExpediente		AS S WITH(NOLOCK)
		ON			S.IDACO								=  X.Y.value('(IDACOSOL)[1]', 'INT')
		AND			S.TC_NumeroExpediente				=  @L_NumeroExpediente
