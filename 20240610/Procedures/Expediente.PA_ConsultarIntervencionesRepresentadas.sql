...
-- =================================================================================================================================================
-- Modificacion				<20-04-2020> <Daniel Ruiz H> <Se modifica consulta para incluir el codigo de representacion>
...
-- =================================================================================================================================================
CREATE PROCEDURE [Expediente].[PA_ConsultarIntervencionesRepresentadas]
...
Begin
	...
	IF (@L_CodigoLegajo IS NULL)
		Select			Z.TF_Inicio_Vigencia			As FechaActivacion,		  
						...
						'Split'							As	Split,
						X.TU_CodIntervinienteRepresentante As CodigoRepresentante,
						...
						X.TN_CodTipoRepresentacion		As	CodigoRepresentacion,		P.TC_Descripcion				As	TipoRepresentacionDescrip,
		...
		Where			(X.TU_CodIntervinienteRepresentante	=ISNULL(@CodigoInterviniente, X.TU_CodIntervinienteRepresentante))
		AND				Y.TC_TipoParticipacion		    =   @TipoParticipacion
		And				(Z.TC_NumeroExpediente			=	ISNULL(@NumeroExpediente, Z.TC_NumeroExpediente))
		And				(
						 (Z.TU_CodPersona				=	Case	When @CodigoPersona is not null And @CodigoInterviniente is null
																	Then @CodigoPersona
																	Else Z.TU_CodPersona
															End)
							And
						 (IsNull(A.TF_Actualizacion,'')			=	Case	When @CodigoPersona is not null And @CodigoInterviniente is null
																	Then (Select MAX(T.TF_Actualizacion) From Expediente.Intervencion T Where T.TU_CodPersona = @CodigoPersona) 
																	Else Isnull(A.TF_Actualizacion,'')
															End)
						)
		And				((@CodigoInterviniente is not null) Or (@NumeroExpediente is not null) Or (@CodigoPersona is not null))
