...
-- =================================================================================================================================================
CREATE PROCEDURE [Expediente].[PA_ConsultarDevolucionesparaCorregir]
...
AS
BEGIN
	IF @CodPuestoTrabajo IS NULL OR @CodPuestoTrabajo =''
	BEGIN
		...
		WHERE 
			...
			A.TC_Estado = ISNULL(@CodEstado, A.TC_Estado) AND
			B.TU_CodArchivo = ISNULL(@CodLegajoArchivo, B.TU_CodArchivo)
	END
	ELSE
	BEGIN		
			SELECT A.[TU_CodAsignacionFirmado]		AS Codigo,
				...
				'Split' AS Split,
				B.TU_CodArchivo						AS  CodigoArchivoFirmado,			
				B.TC_Descripcion					AS Descripcion,
				A.TU_CodArchivoFirmado				AS CodigoFS,						 
				--Asignado por
				'Split' AS Split,
				...
				B.TU_CodLegajo						AS CodigoLegajo,
				E.TC_Descripcion					As DescripcionLegajo,
				P.TC_NumeroExpediente				AS NumeroExpediente,			
				--Clase Asunto
				F.TN_CodClaseAsunto					As	CodigoAsunto,				 
				F.TC_Descripcion					As	DescripcionAsunto,
				...
				A.TC_Estado							AS EstadoAsignacion,
				A.TF_FechaDevolucion			    AS FechaDevolucion
			FROM [Expediente].[AsignacionFirmado] A		
				INNER JOIN [Expediente].LegajoArchivo B WITH(NOLOCK)
				ON A.TU_CodArchivo = B.TU_CodArchivo
				INNER JOIN Catalogo.GrupoTrabajo D WITH(NOLOCK) 
				ON B.TN_CodGrupoTrabajo = D.TN_CodGrupoTrabajo
				INNER JOIN Expediente .Legajo AS P WITH(NOLOCK)
				ON P.TU_CodLegajo = B.TU_CodLegajo
				INNER JOIN Expediente.ExpedienteDetalle AS E WITH(NOLOCK)
				ON E.TU_CodLegajo = P.TU_CodLegajo
				INNER JOIN Expediente.Expediente AS R WITH(NOLOCK)
				ON R.TC_NumeroExpediente = P.TC_NumeroExpediente
				INNER JOIN Catalogo.ClaseAsunto As  F With(Nolock)
				ON F.TN_CodClaseAsunto=R.TN_CodClaseAsunto 
				--Asignador por
				INNER JOIN  Catalogo.PuestoTrabajoFuncionario G WITH(NOLOCK) 
				...
				LEFT JOIN  Catalogo.PuestoTrabajo Q WITH(NOLOCK) 
				ON	A.TU_DevueltoA = Q.TC_CodPuestoTrabajo AND A.TU_DevueltoA = @CodPuestoTrabajo
			WHERE 
				A.TU_CodAsignacionFirmado = ISNULL(@Codigo, A.TU_CodAsignacionFirmado) AND
				B.TU_CodLegajo = ISNULL(@CodLegajo, B.TU_CodLegajo) AND			
				A.TC_Estado = ISNULL(@EstadoDevolver, A.TC_Estado) AND
				B.TU_CodArchivo = ISNULL(@CodLegajoArchivo, B.TU_CodArchivo)
